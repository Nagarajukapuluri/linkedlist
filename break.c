...
int new(struct node *head)
	...
	temp=head;
	create : newnode=(struct node *)malloc(sizeof(struct node));
...
int del(struct node *head)
{
	...
	case 1:while(temp->link!=NULL)
		   ...
		   if(!strcmp(c,"yes"))
		     ...
		     return 0;
	case 2:printf("enter node number to delete\n");
		   scanf("%d",&o);
		   for(int i=1;i<o;i++)
		   {
 			  temp2=temp;
...
int insert(struct node *head)
{
 ...
 repeat :printf("enter node number after which a node is inserted");
 scanf("%d",&n);
 temp=head;
